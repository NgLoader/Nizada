		...
		if(args.length > 0) {
			...
			if(target != null)
				if(target.getUniqueId() != player.getUniqueId())
					if(TpaManager.addRequest(player, target, null)) {
						player.sendMessage(Freebuild.PREFIX + "§7Du hast eine §aTeleportanfrage §aan §8\"§2" + target.getName() + "§8\" §7gestellt§8.");
						TextComponent message = new TextComponent(Freebuild.PREFIX + "§7Du hast eine §aTeleportanfrage §7von §8\"§2" + player.getName() + "§8\" §7erhalten§8.");
						...
						player.sendMessage(Freebuild.PREFIX + "§7Du hast §8\"§c" + target.getName() + "§8\" §7bereits eine §cTeleportanfrage §7gestellt§8.");
				else
			...
			player.sendMessage(Freebuild.PREFIX + "§8/§7tpa §8<§aSpieler§8>");
		return true;
	}
}