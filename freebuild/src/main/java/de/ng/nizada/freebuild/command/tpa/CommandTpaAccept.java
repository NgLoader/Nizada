...
public class CommandTpaAccept implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length > 0) {
			...
			if(target != null) {
				TpaCache tpaCache = TpaManager.hasRequestAndRemove(target.getUniqueId(), player.getUniqueId());

				if(tpaCache != null) {
					player.sendMessage(Freebuild.PREFIX + "§7Die §aTeleportanfrage §7von §8\"§2" + target.getName() + "§8\" §7wurde §aerfolgreich §7angenonnen§8.");
					target.sendMessage(Freebuild.PREFIX + "§7Die §aTeleportanfrage §7an §8\"§2" + player.getName() + "§8\" §7wurde §aerfolgreich §7angenommen§8.");

					if(tpaCache.location != null)
						player.teleport(tpaCache.location.add(0, 0.2, 0));
					else
						target.teleport(player.getLocation().add(0, 0.2, 0));
				} else
			...
			player.sendMessage(Freebuild.PREFIX + "§8/§7tpaaccept §8<§aSpieler§8>");
		return true;
	}
}